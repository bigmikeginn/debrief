...
function buildDisplayKeyPointFallback(rawNotes) {
  ...
  if (/\bgrip|grips|connect|hook|hooks|frame)\b/.test(text)) points.push("Build connection first");
  if (/\blight|timing|when|then|start)\b/.test(text)) points.push("Move when resistance lightens");
  if (/\bfinish|complete|follow through|follow-through)\b/.test(text)) points.push("Follow through to finish");
