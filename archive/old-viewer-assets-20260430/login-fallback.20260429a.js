(function () {
  ...
  if (window.location.pathname.includes("viewer") && (new URLSearchParams(window.location.search).get("login") === "1" || readSession()?.access_token)) {
  ...
  function renderPagination() {
    const previous = document.querySelector("#previousPageButton");
    const next = document.querySelector("#loadMoreButton");
    if (previous) {
      previous.classList.toggle("hidden", fallbackState.page <= 0);
      previous.textContent = "Previous Page";
    }
    if (next) {
      next.classList.toggle("hidden", !fallbackState.hasNext);
  ...
  function setupFallbackControls() {
    ...
    document.querySelector("#previousPageButton")?.addEventListener("click", () => loadFallbackPage(fallbackState.page - 1));
    document.querySelector("#loadMoreButton")?.addEventListener("click", () => loadFallbackPage(fallbackState.page + 1));
