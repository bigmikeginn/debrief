...
function boot() {
  ...
  if (refreshTelegramLinkButton) refreshTelegramLinkButton.addEventListener("click", () => loadTelegramLinkState({ forceNewCode: false }));
...
function setupSupabase() {
  ...
  supabase.auth.onAuthStateChange(async (_event, session) => {
    ...
    if (currentUser) {
      ...
      await loadTelegramLinkState({ forceNewCode: false });
      await loadEntries({ reset: true });
    }
  });
...
function renderAuthState() {
  if (currentUser) {
    ...
    if (telegramLinkCard) telegramLinkCard.classList.remove("hidden");
    setAuthStatus(`Logged in as ${currentUser.email}`, false);
    renderViewState();
  } else {
...
async function loadEntries({ reset }) {
  ...
  let batch = data ?? [];
  if (currentView === "mine" && batch.length === 0 && !dateFrom && !dateTo && !domainFilter && !topicSearch && tagFilters.length === 0) {
    const fallbackLoaded = await loadMyDebriefsFallback({ from, reset });
    if (fallbackLoaded) return;
  }
...
async function loadMyDebriefsFallback({ from, reset }) {
  const { data, error } = await supabase.rpc("get_my_debrief_history", {
