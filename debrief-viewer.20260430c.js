...
const TRIAL_DAYS = 14;
const WHATS_NEW_KEY = "debrief-whats-new-20260430-admin-controls";
...
function handleDelegatedActions(event) {
  ...
  if (actionId === "shareToggleButton") {
    setListStatus("[DEBUG] shareToggleButton click detected, looking up entry...", false);
  }
  if (actionId === "topLogoutButton" || actionId === "logoutButton" || actionId === "logoutButtonCompact") {
    ...
    setListStatus("[DEBUG] activeEntryId=" + activeEntryId + " foundEntry=" + (entry ? entry.id : "NULL"), false);
    if (entry) {
      console.log("Delegated share click for entry:", entry.id, entry.note_title);
      openSharePanel(entry);
    } else {
      setListStatus("[DEBUG] No entry found for share. activeEntryId=" + activeEntryId + " entries.length=" + entries.length, true);
    }
...
function renderWhatsNewNotice() {
  ...
  whatsNewNotice.innerHTML = `
    <div>
      ...
      <strong>Club admins can now remove participants.</strong>
      <p>If someone should no longer be in a club, an admin can remove them from Club access. No action needed unless you manage a club.</p>
    </div>
...
function renderEntryDetail() {
  ...
  // Expose global diagnostic function for share testing
  window.__deb_share = function () {
    const e = entries.find((item) => item.id === activeEntryId);
    console.log("__deb_share: canShare=", canShare, "activeEntryId=", activeEntryId, "found=", !!e, "entries.length=", entries.length);
    if (e) openSharePanel(e);
  };
  // Attach shareToggleButton listener.
  if (canShare) {
    ...
    if (shareToggleButton) {
      shareToggleButton.addEventListener("click", (event) => {
        ...
        console.log("shareToggleButton direct click");
        const entryToShare = entries.find((item) => item.id === activeEntryId);
        if (entryToShare) {
          console.log("Calling openSharePanel for entry:", entryToShare.id);
          openSharePanel(entryToShare);
        } else {
          console.log("No entry found for activeEntryId:", activeEntryId);
        }
      });
    } else {
      console.log("shareToggleButton NOT FOUND in DOM");
    }
  }
...
async function openSharePanel(entry) {
  try {
    ...
    `);
    const wcInput = document.querySelector("#shareWholeClub");
    if (wcInput) wcInput.addEventListener("change", () => { activeShareWholeClub = wcInput.checked; refreshSharePeopleModal(); });
    const saveBtn = document.querySelector("#saveShareSettingsButton");
    if (saveBtn) saveBtn.addEventListener("click", saveShareSettings);
    const stopBtn = document.querySelector("#stopSharingButton");
    if (stopBtn) stopBtn.addEventListener("click", stopSharingActiveEntry);
...
function refreshSharePeopleModal() {
  const list = document.querySelector("#sharePeopleList");
...
async function writeShareSettings(entry, { shareWholeClub, recipientIds }) {
  if (!canShareDebriefs()) {
    setSharePanelStatus("Sharing is available on paid plans.", true);
    return;
  }
