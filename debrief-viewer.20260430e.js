...
function renderEntryDetail() {
  ...
  // shareToggleButton is handled via the delegated document click handler (handleDelegatedActions)
  // to avoid a race with handleGlobalClick which would close the drawer immediately.
...
async function openSharePanel(entry) {
  try {
    ...
    if (!canShareDebriefs()) {
      ...
      if (sharePanelTitle) sharePanelTitle.textContent = "Sharing is a paid feature";
      if (shareWholeClubInput) {
        shareWholeClubInput.checked = false;
        shareWholeClubInput.disabled = true;
      }
      if (sharePeopleList) {
        sharePeopleList.innerHTML = '<p class="muted">Free accounts keep notes private. Upgrade to share with your club or selected teammates.</p>';
      }
      if (saveShareSettingsButton) saveShareSettingsButton.disabled = true;
      if (stopSharingButton) stopSharingButton.disabled = true;
      setSharePanelStatus("Upgrade to unlock sharing.", true);
      return;
    }
    activeShareEntry = entry;
    if (shareWholeClubInput) shareWholeClubInput.disabled = false;
    if (saveShareSettingsButton) saveShareSettingsButton.disabled = false;
    if (stopSharingButton) stopSharingButton.disabled = false;
    if (sharePanel) sharePanel.classList.remove("hidden");
    ...
    setSharePanelStatus(error?.message || "Could not open sharing. Try refreshing the page.", true);
    openMenuPanel();
    if (sharePanel) sharePanel.classList.remove("hidden");
  }
}
